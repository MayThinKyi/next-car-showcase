...
const CarFilter = () => {
    ...
    const filterHandler = (f: string, y: number) => {
        ...
        f && quries.append('fuel', f.toLowerCase())
        y && quries.append('year', y.toString())
        // searchParams.get('limit') && quries.append('limit', searchParams.get('limit')!)
        router.push(`?${quries.toString()}`, { scroll: false })
    }
    return (
        <div className='flex items-center gap-2 sm:gap-10'>
            <div className="w-[130px] sm:w-40">
                <Listbox value={fuel} onChange={(value) => {
                    setFuel(value)
                    filterHandler(value, Number(year))
                }}>
                    <div className="relative mt-1">
                        <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none  sm:text-sm">
                            <span className="block truncate">{fuel || 'Fuel'}</span>
                        ...
                        <Transition
                            ...
                            leaveTo="opacity-0"
                        >
                            <Listbox.Options className="absolute mt-10 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
            ...
            <div className="  w-[120px] sm:w-40">
                <Listbox value={year} onChange={(value) => {
                    setYear(value)
                    filterHandler(fuel, Number(value))
                }}>
                    <div className="relative mt-1">
                        <Listbox.Button className="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-md focus:outline-none  sm:text-sm">
                        ...
                        >
                            <Listbox.Options className="absolute mt-10 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                                {yearsOfProduction.map((item, index) => (
                                    <Listbox.Option
                                        key={index}
                                        className={({ active }) =>
                                            `relative cursor-default select-none py-2 pl-10 pr-4 ${active ? 'bg-blue-100 text-blue-900' : 'text-gray-900'
